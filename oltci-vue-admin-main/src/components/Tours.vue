<template>
  <v-card color="grey-lighten-5">
        ...
        <v-card :title="formTitle">
          <v-card-text class="mt-4">
            ...
            <v-autocomplete
              ...
              :label="$t('Captain')"
              variant="outlined"
              :item-title="item => `${item.first_name} ${item.last_name} (${item.average_rating})`"
              item-value="id"
              density="comfortable"
    ...
    >
      ...
      <template v-slot:item.boat.picture="{ item }">
        <v-img :src="item.boat.picture" width="100" height="100" aspect-ratio="1" cover></v-img>
      </template>
    </v-data-table>
  </v-card>
...
const headers = [
  ...
  { key: 'boat.picture', title: t('Boat Picture')},
  { key: "boat.name", title: t('Boat') },
  { key: "captain", title: t('Captain'), value: (item) => `${item.captain.first_name} ${item.captain.last_name} (${item.captain.average_rating})` },
  { key: "departure.location", title: t('Departure') },
  { key: "destination.location", title: t('Destination') },
  { key: "duration", title: t('Duration (h)') },
